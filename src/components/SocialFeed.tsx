...
/* ------------------------------------------------------------------ */
/* Social feed — reusable (used on the home page AND the gallery page)  */
/* Posts render newest-first: the `galleryPosts` array is curated in    */
/* reverse-chronological order; pinned/featured posts float to the top. */
/* ------------------------------------------------------------------ */
export const SocialFeed = ({ showTabs = true }: { showTabs?: boolean }) => {
  const { lang } = useLang();
  ...
  const posts = useMemo(() => {
    const list = activeCat === "all" ? galleryPosts : galleryPosts.filter((p) => p.category === activeCat);
    // pinned/featured first; otherwise preserve the curated newest-first order
